...
export const createLeaveBalanceUpdateBatch = async (req, res) => {
    try {
        ...
        if (!Array.isArray(lines) || lines.length === 0) {
            return res.status(400).json({ success: false, message: "No lines provided" });
        }

        // Validate each line
        for (const line of lines) {
            const userId = String(line.userId || "").trim();
            const leaveTypeId = String(line.leaveTypeId || "").trim();
            const daysTaken = Number(line.daysTaken);

            if (!userId) {
                return res.status(400).json({ success: false, message: "Missing userId in line" });
            }
            if (!leaveTypeId) {
                return res.status(400).json({ success: false, message: "Missing leaveTypeId in line" });
            }
            if (Number.isNaN(daysTaken) || daysTaken < 0) {
                return res.status(400).json({ success: false, message: "Invalid daysTaken in line" });
            }
        }
        ...
        const normalizedLines = lines.map((line, index) => ({
            lineNumber: index + 1,
            userId: String(line.userId || "").trim(),
            leaveTypeId: String(line.leaveTypeId || "").trim(),
            daysTaken: Number(line.daysTaken) || 0,
        }));

        // Verify all employees exist and belong to tenant
        const userIds = [...new Set(normalizedLines.map((l) => l.userId))];
        const employees = await prisma.user.findMany({
            where: { id: { in: userIds }, tenantId, role: "EMPLOYEE" },
            select: { id: true },
        });
        const validEmployeeIds = new Set(employees.map((e) => e.id));

        // Verify all leave types exist and belong to tenant
        const leaveTypeIds = [...new Set(normalizedLines.map((l) => l.leaveTypeId))];
        const leaveTypes = await prisma.leaveType.findMany({
            where: { id: { in: leaveTypeIds }, tenantId, isActive: true },
            select: { id: true, deductsFromAnnual: true, deductsFromSickAllocation: true },
        });
        ...
        const validLines = normalizedLines.filter((line) => {
            return validEmployeeIds.has(line.userId) && validLeaveTypeIds.has(line.leaveTypeId);
        });

        if (validLines.length === 0) {
            return res.status(400).json({ success: false, message: "No valid employee/leave type combinations found" });
        }
        ...
        const rowCreates = validLines.map((line) => ({
            ...
            rawPayload: {
                ...
                daysTaken: line.daysTaken,
                deductsFromAnnual: leaveTypeById[line.leaveTypeId]?.deductsFromAnnual ?? true,
                deductsFromSickAllocation: leaveTypeById[line.leaveTypeId]?.deductsFromSickAllocation ?? false,
            },
        }));
        ...
        return res.status(201).json({
            success: true,
            data: { id: job.id, batchCode: job.batchCode, totalRows: validLines.length, processingMode },
        });
    } catch (e) {
