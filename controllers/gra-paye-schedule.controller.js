...
/**
 * GET /api/gra-paye-schedules
 * List stored GRA PAYE schedule PDFs for the current tenant (newest first).
 */
export const listGraPayeSchedules = async (req, res) => {
    try {
        ...
        const items = await prisma.graPayeScheduleDocument.findMany({
            where: { tenantId },
        ...
        logger.error(`Error listing GRA PAYE schedules: ${error.message}`, {
            error: error.stack,
        ...
        return res.status(500).json({
            ...
            message: "Failed to list GRA PAYE schedules",
        });
    }
};
